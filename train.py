...
class Trainer_seg:
    def __init__(self, args, now_time=None):
        ...
        train_z_path = self.args.train_z_path if getattr(self.args, 'use_fov', False) else None
    ...
    def _train(self, epoch):
        ...
        print('Start Train')
        for batch_idx, (x_in, target, fov) in enumerate(self.loader_train.Loader):
            ...
            if isinstance(output, tuple) or isinstance(output, list):  # condition for deep supervision
                if getattr(self.args, 'use_fov', False):
                    loss = sum([self.criterion(item * fov, target * fov) for item in output]) / len(output)
                else:
                ...
                if getattr(self.args, 'use_fov', False):
                    loss = self.criterion(output * fov, target * fov)
                else:
            ...
            if getattr(self.args, 'use_fov', False):
                output_np = (output_argmax[:, 0] * fov.squeeze(1)).cpu()
                target_np = (target.squeeze(0) * fov.squeeze(1)).detach().cpu().numpy()
            else:
                output_np = output_argmax[:, 0].cpu()
                target_np = target.squeeze(0).detach().cpu().numpy()
            metric_result = metrics.metrics_np(output_np, target_np, b_auc=False)
    ...
    def _validate(self, model, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (x_in, target, fov) in enumerate(self.loader_val.Loader):
                x_in, _ = x_in
                target, _ = target
                fov, _ = fov
                x_in = x_in.to(self.device)
    ...
    def __init_data_loader(self,
        ...
        elif self.args.dataloader == 'Image2Image_crop':
            loader = dataloader_hub.Image2ImageDataLoader_crop(x_path=x_path,
                                                               y_path=y_path,
                                                               z_path=z_path,
                                                               batch_size=batch_size,
                                                               num_workers=self.args.worker,
