...
def center_padding(img, target_size, pad_digit=0):
    ...
    Args:
        ...
        target_size: list -> (target_w, target_h)
        pad_digit: int
    ...
    assert target_h >= in_h and target_w >= in_w, 'target_size should be larger than input_size to pad'

    pad_left = (target_w - in_w) // 2
    pad_right = pad_left if (target_w - in_w) % 2 == 0 else pad_left + 1
    pad_top = (target_h - in_h) // 2
    pad_bot = pad_top if (target_h - in_h) % 2 == 0 else pad_top + 1
...
def remove_center_padding(img):
    ...
    assert target_h <= in_h and target_w <= in_w, 'target_size should be smaller than input_size'

    pad_left = abs((target_w - in_w) // 2)
    pad_top = abs((target_h - in_h) // 2)

    tensor_unpadded = img[:, :, pad_top:pad_top + target_h, pad_left:pad_left + target_w]
    _, _, out_h, out_w, = tensor_unpadded.shape

    assert target_h == out_h and target_w == out_w, 'target_size should be same with input_size'
